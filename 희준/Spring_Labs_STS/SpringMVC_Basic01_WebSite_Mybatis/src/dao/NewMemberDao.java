...
public class NewMemberDao implements MemberDao {
	...
	public Member getMember(String userid) throws ClassNotFoundException, SQLException {
		...
		// 권장 방법이 아니고 : DTO, (VO) memberfield와 select column명이 같지 않는 경우...
		List<Member> results = jdbctemplate.query(
				"select userid, pwd, name, gender, birth, is_lunar, cphone, email, habit, regdate from member where userid=? ",
				new RowMapper<Member>() {

					@Override
					public Member mapRow(ResultSet rs, int rownum) throws SQLException {
						Member member = new Member(rs.getString("userid"), rs.getString("pwd"), rs.getString("name"),
								rs.getString("gender"), rs.getString("birth"), rs.getString("is_Lunar"),
								rs.getString("cPhone"), rs.getString("email"), rs.getString("habit"),
								rs.getDate("regDate"));
						return member;
					}
				});
