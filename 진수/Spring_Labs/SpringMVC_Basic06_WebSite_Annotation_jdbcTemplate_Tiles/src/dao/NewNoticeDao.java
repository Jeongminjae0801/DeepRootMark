...
NewNoticeDao 의존합니다 JdbcTemplate 객체  (NewNoticeDao가 JdbcTemplate의 주소를 필요로 한다)
...
3. JdbcTemplete 객체 사용시 ****************************** ( 자동화 되기 위한 전제 조건이기에 필수! 따르지 않으면 일반 java 코드와 동일한 복잡함을 가진다 )
   -컬럼명과 VO의 변수명이 같아야한다.
...
public class NewNoticeDao implements NoticeDao { //하위 컨테이너 : 에는 상위 컨테이너를 사용 할 수 있다. ******************* 매우 중요한 개념 ****************

	private JdbcTemplate jdbctemplate; //상위 컨테이너 
	@Autowired
	...
	public List<Notice> getNotices(int page, String field, String query) throws ClassNotFoundException, SQLException {
		...
		return this.jdbctemplate.query(sql, new Object[]{"%"+query+"%",srow,erow}, new BeanPropertyRowMapper<Notice>(Notice.class));  //new BeanPropertyRowMapper<Notice>(Notice.class) // <Notice> 객체만 넣었지만, 알아서 데이터 건수만큼 .. List 추가 ..... 하여 리턴하기 때문에, public List<Notice> 에서 List 컬렌션으로 받는다. 
	}
